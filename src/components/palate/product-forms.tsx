...
export function EditProfileForm({ defaults }: { defaults: ProfileFormDefaults }) {
  ...
  return <form action={action} className="form-modal"><header><div><p className="eyebrow">Your Palate</p><h2>Edit profile</h2></div><Link className="icon-button" href={`/profile/${defaults.username}`}><X /></Link></header><div className="form-scroll"><div className="split-fields"><label className="form-label">Username<input name="username" required defaultValue={defaults.username} /></label><label className="form-label">Display name<input name="displayName" required defaultValue={defaults.displayName} /></label></div><label className="form-label">Bio<textarea name="bio" maxLength={500} defaultValue={defaults.bio} /></label><label className="form-label">Home city<input name="homeCity" defaultValue={defaults.homeCity} /></label><label className="form-label">Favourite cuisines<input name="favouriteCuisines" defaultValue={defaults.favouriteCuisines.join(', ')} /></label><label className="form-label">Disliked cuisines<input name="dislikedCuisines" defaultValue={defaults.dislikedCuisines.join(', ')} /></label><label className="form-label">Favourite vibes<input name="preferredVibes" defaultValue={defaults.preferredVibes.join(', ')} /></label><label className="form-label">Preferred neighbourhoods<input name="preferredNeighbourhoods" defaultValue={defaults.preferredNeighbourhoods.join(', ')} /></label><div className="split-fields"><label className="form-label">Price range<input name="pricePreference" defaultValue={defaults.pricePreference.join(', ')} placeholder="2, 3" /></label><label className="form-label">Visibility<select name="privacy" defaultValue={defaults.privacy}><option value="public">Public</option><option value="followers">Friends</option><option value="private">Private</option></select></label></div><FormMessage state={state} /></div><footer><Link className="button ghost" href={`/profile/${defaults.username}`}>Cancel</Link><SubmitButton className="button wine">Save profile <Check /></SubmitButton></footer></form>
}
